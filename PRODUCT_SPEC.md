...
| 2026-07-25 | §2.5 colour editing: `RivShapePaints` resolves fill/stroke SolidColors through the hierarchy (Shape→Fill→SolidColor, Text→TextStylePaint→Fill→SolidColor); `SetComponentColorCommand` (multi-target, serialisable, mergeable, byte-identity undo) writes ARGB values; `RivRawProperty` gains colorValue accessors. Shared `ColorField` (swatch + hex entry) and `EditorColorPicker` (HSV area, hue slider, alpha slider over checkerboard, hex) in `shared/widgets`; inspector shows a Paint section with live Fill/Stroke editors. Eyedropper tool (I) samples a clicked shape's fill and applies it to the selection in one undo entry. Property grid and canvas now use the shared `RivHierarchy.componentObjects` resolver. | feature/color-editing |
| 2026-07-26 | Canvas interaction polish: overlay layer now repaints on document, selection, and view-transform changes so selection outlines always track shapes (previously they froze while panning/zooming or after edits). Hit tester keyed on a new synchronous `documentRevision` (bumped before the async engine reload) so hit regions always match the bytes a tool just mutated; engine reloads are serialised (in-flight + trailing coalesce) so drag streams can't race concurrent decodes. Figma-style artboard frame: surface plate + border + name/size label painted under the content, artboards created with `clip=true` (key 196) so shapes stay inside the frame. Hierarchy lists siblings topmost-first (draw order) and artboard roots default to expanded. Text hit regions resolve through the live engine component bounds. Auto-fit re-fits while the view is untouched by the user and the viewport resizes. Deterministic draw→select→drag integration test drives the real widget tree. | fix/canvas-interaction-polish |
| 2026-08-10 | §2.7 Phase 2 start, keyframe authoring: `RivAnimationFactory` writes LinearAnimation/KeyedObject/KeyedProperty/KeyFrameDouble into the raw stream (exporter layout, ascending frame order, reuse-or-create per object/property, no component index shifts). `AddAnimationCommand` and `InsertKeyframeCommand` (mergeable per frame so value adjustments coalesce), both serialisable with byte-identity undo. Animations panel gains a New Animation action; inspector numeric rows gain a keyframe-at-playhead button when an animation is selected. Parser fix: animation names read Animation.name (key 55), not Component.name. Engine integration test proves authored keyframes decode and evaluate correctly (linear interpolation verified at mid-frame). | feature/timeline-keyframe-authoring |
| 2026-08-10 | §2.7 keyframe deletion: `RivAnimationFactory.deleteKeyframe` removes a keyframe by raw index and prunes newly-empty KeyedProperty/KeyedObject containers (runtime rejects childless keyed containers). `DeleteKeyframeCommand` serialisable with byte-identity undo. Right-clicking a timeline diamond opens the shared context menu with a destructive Delete action, wired through `EditorState.deleteKeyframe`. `RivTypeKeys.keyFrameTypeKeys` names all KeyFrame subtypes. | feature/timeline-keyframe-delete |
| 2026-08-10 | §2.7 loop modes: `RivLoopMode` (oneShot/loop/pingPong) parsed from LinearAnimation.loopValue and modelled on `RivAnimationModel`; `SetAnimationUintCommand` + `RivAnimationFactory.setAnimationUint` author loop/fps/duration edits (serialisable, byte-identity undo). Timeline transport gains a loop-mode cycle button (Loop → Ping Pong → One Shot), persisted to the document and applied to live playback: the painter now stops at the end for one-shot, wraps for loop, and bounces with a direction flag for ping-pong; replaying a finished one-shot restarts from 0. Painter loop state re-syncs on animation/artboard switches and engine reloads. | feature/timeline-loop-modes |
| 2026-08-10 | §2.7 keyframe copy/paste: timeline context menu gains Copy keyframe (on a diamond) and Paste at playhead (when the clipboard has content). The clipboard captures (objectId, propertyKey, value); paste re-keys through the existing `InsertKeyframeCommand` at the current playhead frame, so it is undoable and merges naturally. Widget tests cover menu composition per clipboard state and hit position. | feature/timeline-keyframe-clipboard |
| 2026-08-11 | Engine-reload data-loss fix + animation shortcuts: `EditorDocument.decode` gains `reuseEditor` and engine reloads now reuse the live byte-level editor instead of re-parsing snapshot bytes — commands dispatched while an async reload was in flight were previously silently discarded (lost keyframes/moves). Shortcuts: `Shift+Cmd/Ctrl+A` creates an animation; `K` keyframes the primary selection's X/Y at the playhead, auto-creating a default animation when none exists. SHORTCUTS.md documents the new Animation section. | fix/engine-reload-mutation-loss ||
| 2026-08-11 | §2.8 interpolation modes: keyframe context menu gains Linear / Hold (stepped) with a checkmark on the active mode. `SetKeyframeInterpolationCommand` writes interpolationType (key 68), serialisable with byte-identity undo, restricted to interpolator-free types (cubic needs interpolator objects and lands with tangent tooling). Curve editor and playback already honour hold segments via the shared evaluator and the runtime. | feature/keyframe-interpolation |
| 2026-08-11 | §2.8 cubic ease interpolation: keyframe context menu gains Ease (cubic), applying a standard ease-in-out via new `SetKeyframeCubicCommand` (mergeable per keyframe, serialisable, byte-identity undo). `RivAnimationFactory.setKeyframeCubic` reuses the keyframe's existing CubicEaseInterpolator or appends one at the end of the artboard span (component indices never shift; runtime resolves interpolator references post-import). Parser resolves interpolatorId (key 69) → `RivCubicEase` control points; the shared evaluator now implements the runtime's `CubicInterpolatorSolver` (Newton-Raphson + bisection) so curve display matches playback exactly — verified by an engine integration test asserting engine x equals evaluator x at t=0.25. | feature/cubic-interpolation |
| 2026-08-11 | §2.8 bezier tangent handles: cubic segments in the curve editor draw stems + orange square handles at both bezier control points (segment space). Handles drag live through mergeable `SetKeyframeCubicCommand` (one drag = one undo entry); x clamps to [0,1] as the runtime requires, y may overshoot for bounce eases. Handle drags take hit priority over control points and re-resolve by raw index across document rebuilds. Widget tests drive real pan gestures over the handle position and assert only the dragged control point changes. | feature/curve-tangent-handles |
| 2026-08-11 | §2.8 multi-channel overlay: the curve editor overlays every animated channel of the object behind the active curve, dimmed, each in a stable per-channel colour (keyed by Rive property key). Header gains colour-dotted channel chips; clicking a chip switches the active editable channel (`EditorState.selectCurveChannel`). The value axis expands to fit all overlaid channels so relative motion is visible. Widget tests verify overlays don't break active-channel drags and colours are stable per key. | feature/curve-multi-channel |
| 2026-08-11 | §2.8 box-select + group transform: dragging on empty curve space draws a marquee that selects control points; dragging any selected point moves the whole group (frames and values together) through new atomic `TransformKeyframesCommand` — batch-validated (a stale index rejects the whole batch), mergeable so one group drag is one undo entry, group frame delta clamped to [0, duration]. Selected points highlight white; marquee renders accent-tinted. Widget test drives marquee + group drag and asserts all three points move by the same frame delta. §2.8 acceptance list complete: shared evaluation, monotonicity, tangent handles, multi-channel overlay, box-select. | feature/curve-box-select |
| 2026-08-11 | §2.7 Scale keyframes + 10k scrub acceptance: Alt/Option+dragging a box-selected group scales frames around the group's first frame (factor clamped to keep every member inside [0, duration]; anchor frame never moves), reusing atomic `TransformKeyframesCommand`. Evaluator hot path optimised for the §2.7 60fps budget: `RivKeyedPropertyModel.numericKeyframes` caches the numeric filter (model is rebuilt on edits; `invalidateCache` covers in-place mutation) and evaluation binary-searches the segment instead of scanning. New benchmark test sweeps 1,000 playhead positions over a 10k-keyframe track best-of-5 in under 8ms. SHORTCUTS.md documents curve-editor gestures. | feature/keyframe-scale-and-perf |
| 2026-08-10 | §2.8 curve editor (first cut): per-property curve view opened from a chart icon on any timeline track row. Plots value-vs-time by sampling `RivKeyframeEvaluator` per horizontal pixel — the same code path used for inspector values (shared-evaluation acceptance). Control points drag horizontally to retime (clamped strictly between neighbour frames, satisfying the time-monotonicity acceptance) and vertically to set values via new mergeable `SetKeyframeValueCommand` (serialisable, byte-identity undo). Timeline toggles between dope-sheet and curve modes with a back header; the curve track is addressed structurally (objectId, propertyKey) so it survives model rebuilds. Widget tests drive real drags to verify the neighbour clamp and vertical value mapping. | feature/curve-editor ||
| 2026-08-10 | §2.8 curve editor (first cut): per-property curve view opened from a chart icon on any timeline track row. Plots value-vs-time by sampling `RivKeyframeEvaluator` per horizontal pixel — the same code path used for inspector values (shared-evaluation acceptance). Control points drag horizontally to retime (clamped strictly between neighbour frames, satisfying the time-monotonicity acceptance) and vertically to set values via new mergeable `SetKeyframeValueCommand` (serialisable, byte-identity undo). Timeline toggles between dope-sheet and curve modes with a back header; the curve track is addressed structurally (objectId, propertyKey) so it survives model rebuilds. Widget tests drive real drags to verify the neighbour clamp and vertical value mapping. | feature/curve-editor ||
